  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-slate-400 mt-1">Live from Zoho CRM: AI-prioritized tasks based on deal value, risk, and urgency.</p>
      </div>

      <div className="space-y-4">
        {actions.map(action => (
          <div key={action.id} className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm flex items-center justify-between group hover:border-slate-700 transition-colors">
            <div className="flex items-start gap-4">
              <div className="mt-1">
                <AlertCircle className={`w-5 h-5 ${action.priority === 1 ? 'text-rose-500' : action.priority === 2 ? 'text-amber-500' : 'text-blue-500'}`} />
              </div>
              <div>
                <h3 className="text-white font-bold">{action.type}</h3>
                <p className="text-slate-300 text-sm mt-1">{action.opp.dealName} <span className="text-slate-500">• {formatCurrency(action.opp.expectedRevenue)}</span></p>
            ...
            <a 
              ...
              className="hidden group-hover:flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm"
            >
        ...
        {actions.length === 0 && (
          <div className="text-center py-12 bg-slate-900/50 rounded-xl border border-slate-800 border-dashed">
            <p className="text-slate-400">You are all caught up! No urgent actions required today.</p>
          </div>
        )}
      </div>
    </div>
