  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-slate-400 mt-1">Live from Zoho CRM: Review missing mandatory fields to ensure strict CRM discipline.</p>
      </div>
...
function QualificationCard({ opp }) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm flex flex-col md:flex-row md:items-start justify-between gap-6">
      <div className="flex-1 space-y-4">
        <div className="flex items-center gap-3">
          <h3 className="text-lg font-bold text-white">{opp.dealName}</h3>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p className="text-white font-medium">{formatCurrency(opp.expectedRevenue)}</p>
          </div>
          <div>
            <p className="text-slate-500">Stage</p>
            <p className="text-white font-medium">{opp.stage}</p>
          </div>
        </div>

        {missingFields.length > 0 && (
          <div className="mt-4 p-4 bg-slate-800/50 rounded-lg border border-slate-700/50">
            <p className="text-sm font-medium text-slate-300 mb-2">Missing Mandatory Fields ({missingFields.length}):</p>
            <div className="flex flex-wrap gap-2">
              {missingFields.map(field => (
                <span key={field} className="px-2.5 py-1 bg-slate-800 text-rose-400 border border-rose-900/30 rounded text-xs font-medium">
      ...
      <div className="flex flex-col items-end justify-between self-stretch">
        ...
        <a 
          ...
          className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm shadow-blue-900/20 mt-4 text-center"
        >
          Requalify in CRM
        </a>
      </div>
